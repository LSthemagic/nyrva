...
fn antigravity_reports_independent_buckets_and_relative_reset() {
    let s = parse(json!({"quota":{"gemini-weekly":{"remaining_fraction":0.75,"reset_in_seconds":3600},"claude-5h":{"remaining_fraction":0.2}}}));
...
fn invalid_fraction_is_rejected_not_clamped() {
    let s = parse(json!({"quota":{"bad":{"remaining_fraction":1.5},"negative":{"remaining_fraction":-0.2},"good":{"remaining_fraction":1.0}}}));
    assert_eq!(s.buckets.len(), 1);
    assert_eq!(s.buckets[0].id, "good");
}
#[test]
fn absolute_reset_takes_precedence_and_elapsed_reset_is_not_replenished() {
    let s = parse(json!({"quota":{"weekly":{"remaining_fraction":0.15,"reset_time":"2027-01-15T08:00:00Z","reset_in_seconds":42}}}));
...
fn privacy_allowlist_discards_credentials_prompts_email_and_absolute_paths() {
    let s = parse(json!({"quota":{"weekly":{"remaining_fraction":0.5}},"model":{"id":"gemini"},"context_window":{"used_percentage":42.0,"context_window_size":1000000},"email":"PRIVATE@example.com","access_token":"TOP_SECRET","prompt":"CONFIDENTIAL","workspace":{"current_dir":"C:\\private\\project"}}));
    let text = serde_json::to_string(&s).unwrap();
    for forbidden in ["PRIVATE", "TOP_SECRET", "CONFIDENTIAL", "private", "current_dir"] { assert!(!text.contains(forbidden), "leaked {forbidden}"); }
...
fn control_characters_cannot_reach_terminal_labels() {
    let s = parse(json!({"quota":{"weekly":{"remaining_fraction":0.5}},"model":{"id":"safe\u{001b}[31m\ntext"}}));
