...
pub fn forecast(
    ...
    if samples.len() < 3 || !reserve.is_finite() || !(0.0..=1.0).contains(&reserve) {
        return None;
    }
    let identity = samples.last()?;
    if samples.iter().any(|s| {
        s.validate().is_err()
            || s.provider != identity.provider
            || s.account_id != identity.account_id
            || s.source != identity.source
    }) {
        return None;
    }
    let mut ordered = samples.iter().collect::<Vec<_>>();
    ordered.sort_by_key(|s| s.observed_at_ms);
    ...
    if last.effective_status(now_ms) != DataStatus::Live || last.observed_at_ms > now_ms {
        return None;
    }
    ...
    for s in &ordered {
        if s.validate().is_err()
            ...
            || s.account_id != last.account_id
            || s.source != last.source
            || s.status != DataStatus::Live
        {
            return None;
        }
        let b = s.buckets.iter().find(|b| b.id == bucket_id)?;
        if b.resets_at_ms != Some(reset) || b.estimated || b.count.is_some() {
            return None;
        }
    ...
    let exhaustion = if rate > 0.0 {
        ...
        if duration.is_finite() && duration <= MAX_TIMESTAMP_MS as f64 {
            last.observed_at_ms
                .checked_add(duration.round() as u64)
                .filter(|t| *t <= MAX_TIMESTAMP_MS)
        } else {
            None
        }
    } else {
    ...
    Some(Forecast {
        fraction_per_hour: rate,
        budget_per_hour: budget,
        estimated_exhaustion_at_ms: exhaustion,
