use crate::{model::clean_text, validate_account, Bucket, ContextUsage, DataStatus, Provider, SessionMetadata, Snapshot, MAX_BUCKETS, MAX_PAYLOAD_BYTES, MAX_TIMESTAMP_MS};
...
fn context(v: &Value) -> Option<ContextUsage> {
    ...
    let size = c.get("context_window_size").and_then(Value::as_u64).filter(|v| *v > 0 && *v <= 9_007_199_254_740_991);
    ...
    if used_percentage.is_none() && size.is_none() && input_tokens.is_none() && output_tokens.is_none() { None } else { Some(ContextUsage { used_percentage, size, input_tokens, output_tokens }) }
}
fn session(v: &Value) -> Option<SessionMetadata> {
    let id = text(v.get("session_id"));
    let parent_id = text(v.get("parent_session_id"));
    let project = v.pointer("/workspace/project_dir").or_else(|| v.pointer("/workspace/current_dir")).and_then(Value::as_str).and_then(|s| s.rsplit(['/', '\\']).find(|p| !p.is_empty())).map(clean_text);
...
pub fn parse_statusline(provider: Provider, account: &str, input: &str, now_ms: u64) -> Result<Snapshot, String> {
    ...
    if provider != Provider::Antigravity { return Err("statusline ingestion is not implemented for this provider".into()); }
    ...
    if let Some(quota) = v.get("quota").and_then(Value::as_object) {
        ...
        for (id, value) in quota {
            ...
            if id.is_empty() || id.len() > 512 || id.chars().any(char::is_control) { out.dropped_fields += 1; continue; }
...
pub fn parse_legacy(provider: Provider, account: &str, value: &Value) -> Result<Snapshot, String> {
    ...
    out.status = match value.get("status").and_then(Value::as_str) {
        Some("ok") => DataStatus::Live, Some("stale") => DataStatus::Stale,
        Some("needsAuth") => DataStatus::NeedsAuth, Some("backoff") => DataStatus::Backoff,
        Some("error") => DataStatus::Error, _ => DataStatus::Unavailable,
    };
    if let Some(windows) = value.get("windows").and_then(Value::as_array) {
        ...
        for w in windows {
            let Some(id) = w.get("id").and_then(Value::as_str).filter(|s| !s.is_empty() && s.len() <= 512 && !s.chars().any(char::is_control)) else { out.dropped_fields += 1; continue; };
