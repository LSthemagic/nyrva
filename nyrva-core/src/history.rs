...
impl History {
    pub fn open(path: &Path) -> Result<Self, String> {
        ...
        if version > DATABASE_VERSION { return Err("telemetry database was created by a newer Nyrva".into()); }
        if version == 0 {
    ...
    pub fn open_readonly(path: &Path) -> Result<Self, String> {
        ...
        let version: i64 = conn.query_row("PRAGMA user_version", [], |r| r.get(0))
            .map_err(|_| "cannot read telemetry schema".to_string())?;
        if version != DATABASE_VERSION { return Err("unsupported telemetry database version".into()); }
        Ok(Self { conn })
    }
    ...
    pub fn record(&self, snapshot: &Snapshot) -> Result<bool, String> {
        ...
        let inserted = tx.execute("INSERT OR IGNORE INTO observations
            (provider,account_id,source,observed_at_ms,payload) VALUES (?1,?2,?3,?4,?5)",
            params![snapshot.provider.as_str(), snapshot.account_id, snapshot.source, snapshot.observed_at_ms, payload])
            .map_err(|_| "cannot write telemetry observation")? > 0;
        if inserted {
            tx.execute("DELETE FROM observations WHERE (provider,account_id,source,observed_at_ms) IN
                (SELECT provider,account_id,source,observed_at_ms FROM observations
                 ORDER BY observed_at_ms DESC LIMIT -1 OFFSET ?1)", [MAX_ROWS])
    ...
    pub fn latest(&self, now_ms: u64) -> Result<Vec<Snapshot>, String> {
        let mut stmt = self.conn.prepare("SELECT o.payload FROM observations o WHERE NOT EXISTS
            ...
            ORDER BY o.provider,o.account_id,o.source").map_err(|_| "cannot query current telemetry")?;
    ...
    pub fn samples(&self, provider: Provider, account: &str, since_ms: u64, limit: usize) -> Result<Vec<Snapshot>, String> {
        ...
        if limit == 0 || limit > 5_000 || since_ms > MAX_TIMESTAMP_MS { return Err("invalid history query range".into()); }
        let mut stmt = self.conn.prepare("SELECT payload FROM observations WHERE provider=?1 AND account_id=?2
            AND observed_at_ms>=?3 ORDER BY observed_at_ms DESC,source LIMIT ?4").map_err(|_| "cannot query telemetry history")?;
        let rows = stmt.query_map(params![provider.as_str(), account, since_ms, limit as i64], |r| r.get::<_, String>(0))
            .map_err(|_| "cannot read telemetry history")?;
        let mut out = Vec::new();
        for row in rows { out.push(decode(&row.map_err(|_| "cannot read telemetry row")?)?); }
        out.reverse(); Ok(out)
    }
    ...
    pub fn prune(&self, before_ms: u64) -> Result<usize, String> {
        ...
        self.conn.execute("DELETE FROM observations WHERE observed_at_ms<?1", [before_ms])
...
fn check_regular_file(path: &Path) -> Result<(), String> {
    ...
    if !meta.file_type().is_file() { return Err("telemetry database must be a regular file, not a link".into()); }
    Ok(())
}
