...
Download `Nyrva_1.0.1_x64-setup.exe` and run the installer. The package includes the `nyrva.exe` desktop app, `nyrva-telemetry.exe` CLI and `nyrva-hook.exe` helper.
...
```bash
sudo apt install ./Nyrva_1.0.1_amd64.deb
```
